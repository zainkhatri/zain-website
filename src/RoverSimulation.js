...
const getPathToB = () => {
  const targetWaypoint = waypoints[1]; // Using the waypoints from parent scope
  ...
  // Mark all obstacles on the grid
  for (let obstacle of obstacles) {
    const obsX = obstacle.x * p.width;
    const obsY = obstacle.y * p.height;
    const radius = obstacle.size / 2 + rover.size / 2;
...
const getPathToB = () => {
  ...
  // Create a higher resolution grid for more accurate pathfinding
  const gridSize = 15; // Smaller grid cells for better accuracy
  const gridWidth = Math.ceil(p.width / gridSize);
  const gridHeight = Math.ceil(p.height / gridSize);
  ...
  // Breadth-first search to find path to B
  const queue = [{x: startGridX, y: startGridY, path: []}];
  const visited = new Set();
  visited.add(`${startGridX},${startGridY}`);
  ...
  // For visualization - uncomment to debug path
  // p.fill(0, 255, 0, 50);
  // for (let y = 0; y < gridHeight; y++) {
  //   for (let x = 0; x < gridWidth; x++) {
  //     if (grid[y][x] === 0) {
  ...
  while (queue.length > 0) {
    ...
    // Check if we reached the target
    if (current.x === targetGridX && current.y === targetGridY) {
      // Path found! Return the full path
      return {
        complete: true,
        path: current.path,
        x: targetGridX * gridSize + gridSize / 2,
    ...
    for (const dir of directions) {
      ...
      if (nextX >= 0 && nextX < gridWidth && 
        ...
        const newPath = [...current.path, {
          x: nextX * gridSize + gridSize / 2, 
          y: nextY * gridSize + gridSize / 2
        }];
